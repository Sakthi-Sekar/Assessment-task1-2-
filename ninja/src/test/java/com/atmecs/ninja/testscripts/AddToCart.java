...
//select product using data provider
	...
	public void enterData(String[] in) {
		...
		driver.findElement(By.xpath("loc.button.search.xpath")).sendKeys(Keys.CLEAR);
		...
		Xpath = read.readPropertiesFile("loc.clickitembutton.xpath", ConstantPaths.LOCATORS_FILE);
		PageActions.clickOnElement(driver, Xpath);

		Xpath = read.readPropertiesFile("loc.quantity.xpath", ConstantPaths.LOCATORS_FILE);
		PageActions.clickOnElement(driver, Xpath);
	}
